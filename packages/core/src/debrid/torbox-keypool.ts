...
const EDGE_WINDOW_MS = 60 * 1000; // 1 minute
...
class TorboxKeyPool {
  ...
  /**
   * Select the best key and record usage synchronously (guaranteed correct
   * distribution). Logging is deferred to process.nextTick to keep the
   * request pipeline fast.
   */
  selectKey(): string {
    const now = Date.now();
    ...
    for (const k of this.keys) {
      ...
      if (k.health !== 'healthy') {
        if (now - k.lastErrorAt >= RECOVERY_TIMEOUT_MS) {
          ...
          process.nextTick(() =>
            logger.info(`Key auto-recovered: ${masked}`)
          );
        }
      } else {
        healthyKeys.push(k);
    ...
    if (healthyKeys.length > 0) {
      ...
      // Record usage immediately (synchronous — ensures correct distribution)
      pick.usageTimes.push(now);

      // Defer logging off the hot path
      const selectedKey = pick.key;
      process.nextTick(() =>
        logger.info(this.formatPoolStatus(Date.now(), selectedKey))
      );
...
/**
 * Select a key from the pool with guaranteed even distribution.
 * Key selection and usage recording are synchronous (correct ordering).
 * Only logging is deferred to process.nextTick (keeps request pipeline fast).
...
/**
 * Record an error for a key and invalidate the selection cache so
 * subsequent requests immediately get a different key.
 */
export function recordPoolError(
  ...
  const pool = getKeyPool(rawCredential);
  if (pool) {
    pool.recordError(usedKey, statusCode);
    // Invalidate cache so next request doesn't reuse the errored key
    selectionCache.delete(rawCredential);
  }
}
